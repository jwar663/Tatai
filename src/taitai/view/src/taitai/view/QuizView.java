...
public class QuizView {
	...
	private String _wordSaid, _question;
	private CurrentQuiz _cq;

	/*
	 * constructor
	 */
	public QuizView(CurrentQuiz cq) {
		_questionNumber = cq.getQuestionNumber();
		_firstTry = cq.isFirstTry();
		_clickedRecord = false;
		_level = cq.getLevel();
		_numCorrect = cq.getNumcorrect();
		_question = cq.getQuestion();
		...
		_submit.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
					if (TaitaiModel.pronouncedCorrectlyBoolean(_wordSaid, TaitaiModel.getWordRequired(_number))) {
						_correct = true;
						_cq.incrementNumCorrect();
					} else {
						_correct = false;
					}
					FeedBackView fbv = new FeedBackView(_cq, _correct);
