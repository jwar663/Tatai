...
public class FeedBackView {
	...
	int _questionNumber, _width, _height, _level;
	
	public FeedBackView(boolean firstTry, int questionNumber, int level, boolean correct) {
		_correct = correct; 
		_firstTry = firstTry; 
	...
	public Scene getFeedBackView(int width, int height) {
		...
		toQuestion.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) { // make questions asked appropriate
				if (_correct && !_last) {
					QuizView qv = new QuizView(true,  _questionNumber + 1, _level);
					Taitai.changeScene(qv.getQuizView(_width, _height));
					//cont
				} else if (_firstTry && !_correct) {
					QuizView qv = new QuizView(false, _questionNumber, _level);
					Taitai.changeScene(qv.getQuizView(_width, _height));
					// cont
				} else if (_last) {
					if (_level == 1) { // && got 8 out of 10
						FinishedView fv = new FinishedView(true);
						Taitai.changeScene(fv.getFinishedView(_width, _height));
					} else {
						FinishedView fv = new FinishedView(false);
						Taitai.changeScene(fv.getFinishedView(_width, _height));
					}
				} else {
					QuizView qv = new QuizView(true,  _questionNumber + 1, _level);
